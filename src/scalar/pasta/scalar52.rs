...
//! This code is copied from curve25519-dalek 4.1.2
//! Although this code aims for curve25519 scalar, it could be used for pallas scalar
//! since its mode is less than 2^260.
//!
...
/// TODO: need to change L
/// L is the order of base point, i. e. 0x40000000000000000000000000000000224698fc0994a8dd8c46eb2100000001
...
mod test {
    ...
    use crate::arithmetic::fields::sbb;
    ...
    fn fq_to_bytes(fq: &Fq) -> [u8; 32] {
        let mut res = [0; 32];
        res[..8].copy_from_slice(&fq.0[0].to_le_bytes());
        res[8..16].copy_from_slice(&fq.0[1].to_le_bytes());
        res[16..24].copy_from_slice(&fq.0[2].to_le_bytes());
        res[24..32].copy_from_slice(&fq.0[3].to_le_bytes());

        res
    }

    // 2d41af7b9cb7147797a99bc3c95d18d2ded9a11db5c0e68f5564a0bf00000edb
    #[test]
    fn from_bytes() {
        let modulus = Fq([
            0x5564a0bf00000edb,
        ...
        let modulus_bytes = fq_to_bytes(&modulus);
        let fq_modulus_scalar52 = Scalar52::from_bytes(&modulus_bytes);
        println!("{:x?}", fq_modulus_scalar52.0);

        let return_bytes = fq_modulus_scalar52.as_bytes();
        assert_eq!(modulus_bytes, return_bytes);
    }
}
